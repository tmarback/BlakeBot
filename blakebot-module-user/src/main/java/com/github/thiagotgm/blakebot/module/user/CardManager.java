...
import com.github.thiagotgm.bot_utils.storage.translate.MapTranslator;
...
public class CardManager {
    ...
    /**
     * Executor used to perform card editing operations. Tasks are keyed by the
    ...
    private CardManager() {
        ...
        cardMap = Graphs.synchronizedTree( DatabaseManager.getDatabase().getDataTree( "CustomCards",
                new StringTranslator(), new StorableTranslator<>( () -> new Card() ) ) );
        userMap = Collections.synchronizedMap( DatabaseManager.getDatabase().getDataMap( "UserCustomCards",
                new StringTranslator(), new StorableTranslator<>( () -> new UserCards() ) ) );
     ...
     * @return The card, or <tt>null</tt> if there is no card owned by the given
     *         user with the given title.
     ...
     * ensure no race conditions occur for multiple calls on the same user across
     * different threads. If calls to this method are parallelized, is not necessary
     * for the caller to synchronize those calls.
     * 
     * @param user
     *            The user to add the card to.
     * @param cardTitle
     *            The card title.
     * @return <tt>true</tt> if added successfully. <tt>false</tt> if the user is
     *         already fully using his/hers current card allowance.
     * @throws NullPointerException
     *             if either argument is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the title is longer than the {@link Card#MAX_TITLE_LENGTH
     *             maximum title length}, or if the user already has a card with the
     *             given title. The exception's detail message will be an error
     *             message that indicates the error.
     */
    public boolean addCard( IUser user, String cardTitle ) throws NullPointerException, IllegalArgumentException {
        ...
        try {
            return EXECUTOR.submit( userID, () -> {
                ...
                cardMap.put( card, userID, cardTitle );
     ...
     * 
     * @param user
     *            The user to remove the card from.
     * @param cardTitle
     *            The card title.
     * @return <tt>true</tt> if removed successfully. <tt>false</tt> if the user
     *         does not have any cards with the given title.
     * @throws NullPointerException
     *             if either argument is <tt>null</tt>.
     */
     ...
     *            The user that owns the card.
     * @param curTitle
     *            The current title of the card.
     * @param newTitle
     *            The title to change the card to.
     * @return <tt>true</tt> if changed successfully. <tt>false</tt> if the user
     *         does not have any cards with the given title (<tt>curTitle</tt>).
     * @throws NullPointerException
     *             if either argument is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the new title is longer than the {@link Card#MAX_TITLE_LENGTH
     *             maximum title length}, or if the user already has a card with the
     *             given new title. The exception's detail message will be an error
     *             message that indicates the error.
     */
    public boolean setCardTitle( IUser user, String curTitle, String newTitle )
        ...
        try {
            return EXECUTOR.submit( userID, () -> {
                ...
                cardMap.put( card, userID, newTitle ); // Insert with new title key.
                cards.addCard( card );
     ...
     * 
     * @param user
     *            The user to purchase a slot for.
     * @return <tt>true</tt> if the purchase was successful. <tt>false</tt> if the
     *         given user does not own enough currency to purchase an extra slot.
     * @throws NullPointerException
     *             if the user is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the user already has the maximum amount of card slots. The
     *             exception's detail message will be an error message that
     *             indicates the error.
     */
     ...
     *            The user that owns the card.
     * @param cardTitle
     *            The title of the card to edit.
     * @param fieldName
     *            The name of the field. If there is no field with this name, and
     *            <tt>fieldText</tt> is not <tt>null</tt>, one is created.
     * @param fieldText
     *            The text to set to the field. If <tt>null</tt>, the field with the
     *            given name is deleted.
     * @return If <tt>fieldText</tt> is not <tt>null</tt>: <tt>true</tt> if the
     *         field set successfully, <tt>false</tt> if there was no field with the
     *         given name and this card already has the {@link Card#MAX_FIELDS
     *         maximum amount of fields}.<br>
     *         If <tt>fieldText</tt> is <tt>null</tt>: <tt>true</tt> if the field
     *         with the given name was deleted successfully, <tt>false</tt> if there
     *         was no field with the given name.
     * @throws NullPointerException
     *             if <tt>user</tt>, <tt>cardTitle</tt>, or <tt>fieldName</tt> is
     *             <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if there is no card with the given title for the user, or if
     *             <tt>fieldName</tt> or <tt>fieldText</tt> is an empty string, only
     *             contains white space, or exceeds the
     *             {@link Card#MAX_FIELD_NAME_LENGTH maximum name size} or
     *             {@link Card#MAX_FIELD_TEXT_LENGTH maximum text size},
     *             respectively. The exception's detail message will be an error
     *             message that indicates what the error was.
     * 
     */
    public boolean setField( IUser user, String cardTitle, String fieldName, String fieldText )
        ...
        try {
            return EXECUTOR.submit( userID, () -> {
                ...
                cardMap.put( card, userID, cardTitle ); // Update card.
     ...
     *            The title of the card to edit.
     * @param setter
     *            The operation that sets the attribute.
     * @return <tt>true</tt> if the attribute was set. <tt>false</tt> if the given
     *         user does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the <tt>setter</tt> threw such an exception.
     */
     ...
     *            The title of the card to edit.
     * @param description
     *            The description to be set. If <tt>null</tt>, the current
     *            description is removed.
     * @return <tt>true</tt> if the description was set. <tt>false</tt> if the given
     *         user does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the given description is an empty string, only contains white
     *             space, or exceeds the {@link Card#MAX_DESCRIPTION_LENGTH maximum
     *             description size}. The exception's detail message will be an
     *             error message that indicates what the error was.
     */
     ...
     *            The title of the card to edit.
     * @param url
     *            The URL to be set. If <tt>null</tt>, the current URL is removed.
     * @return <tt>true</tt> if the URL was set. <tt>false</tt> if the given user
     *         does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     */
    public boolean setUrl( IUser user, String cardTitle, String url ) throws NullPointerException {
    ...
    /**
     * Sets the footer of the given card.
     * <p>
     * The operation is internally executed with the appropriate mechanisms to
     * ensure no race conditions occur for multiple calls on the same user across
     * different threads. If calls to this method are parallelized, is not necessary
     ...
     *            The title of the card to edit.
     * @param footer
     *            The footer to be set. If <tt>null</tt>, the current footer is
     *            removed.
     * @return <tt>true</tt> if the footer was set. <tt>false</tt> if the given user
     *         does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the given footer is an empty string, only contains white
     *             space, or exceeds the {@link Card#MAX_FOOTER_LENGTH maximum
     *             footer size}. The exception's detail message will be an error
     *             message that indicates what the error was.
     */
    public boolean setFooter( IUser user, String cardTitle, String footer )
            throws NullPointerException, IllegalArgumentException {

        return setAttribute( user, cardTitle, card -> card.setFooter( footer ) );

    }

    /**
     * Sets the icon of the footer of the given card (shown next to the foooter).
     * <p>
     * The operation is internally executed with the appropriate mechanisms to
     * ensure no race conditions occur for multiple calls on the same user across
     * different threads. If calls to this method are parallelized, is not necessary
     ...
     *            The title of the card to edit.
     * @param iconUrl
     *            The URL of the icon to be set. If <tt>null</tt>, the current icon
     *            is removed.
     * @return <tt>true</tt> if the icon was set. <tt>false</tt> if the given user
     *         does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     */
    public boolean setFooterIcon( IUser user, String cardTitle, String iconUrl ) throws NullPointerException {

        return setAttribute( user, cardTitle, card -> card.setFooterIcon( iconUrl ) );
     ...
     *            The title of the card to edit.
     * @param imageUrl
     *            The URL of the image to be set. If <tt>null</tt>, the current
     *            image is removed.
     * @return <tt>true</tt> if the image was set. <tt>false</tt> if the given user
     *         does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     */
    public boolean setImage( IUser user, String cardTitle, String imageUrl ) throws NullPointerException {
    ...
    /**
     * Sets the thumbnail of the given card (small image attached in the top right).
     * <p>
     * The operation is internally executed with the appropriate mechanisms to
     * ensure no race conditions occur for multiple calls on the same user across
     * different threads. If calls to this method are parallelized, is not necessary
     ...
     *            The title of the card to edit.
     * @param imageUrl
     *            The URL of the image to be set as the thumbnail. If <tt>null</tt>,
     *            the current thumbnail is removed.
     * @return <tt>true</tt> if the thumbnail was set. <tt>false</tt> if the given
     *         user does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     */
    public boolean setThumbnail( IUser user, String cardTitle, String imageUrl ) throws NullPointerException {
     ...
     *            The title of the card to edit.
     * @param name
     *            The name of the author to be set. If <tt>null</tt>, the current
     *            author is removed.
     * @return <tt>true</tt> if the author was set. <tt>false</tt> if the given user
     *         does not have a card with the given title.
     * @throws NullPointerException
     *             if the user or card title is <tt>null</tt>.
     * @throws IllegalArgumentException
     *             if the given author name is an empty string, only contains white
     *             space, or exceeds the {@link Card#MAX_AUTHOR_LENGTH maximum
     *             author name size}. The exception's detail message will be an
     *             error message that indicates what the error was.
     */
    ...
    /**
     * Sets the URL of the author of the given card (that the author name is a link
     * to).
     * <p>
     * The operation is internally executed with the appropriate mechanisms to
     * ensure no race conditions occur for multiple calls on the same user across
     ...
     *             if the user or card title is <tt>null</tt>.
     */
    public boolean setAuthorUrl( IUser user, String cardTitle, String url ) throws NullPointerException {
     ...
     *             if the user or card title is <tt>null</tt>.
     */
    public boolean setAuthorIcon( IUser user, String cardTitle, String iconUrl ) throws NullPointerException {
    ...
    /**
     * A custom card that can be set up by a user and displayed as an embed.
     * 
     * @author ThiagoTGM
     * @version 1.0
     * @since 2018-09-08
    ...
    public static class Card implements Storable {
        ...
        /**
         * Maximum length of the {@link #setDescription(String) description}, in
         * characters.
         */
        public static final int MAX_DESCRIPTION_LENGTH = 512;
        /**
         * Maximum length of a {@link #setField(String,String) field}'s name, in
         * characters.
         */
        public static final int MAX_FIELD_NAME_LENGTH = 128;
        /**
         * Maximum length of a {@link #setField(String,String) field}'s text, in
         * characters.
         */
         ...
         *            The title of the card.
         * @throws NullPointerException
         *             if the given title is <tt>null</tt>.
         * @throws IllegalArgumentException
         *             if the given title is empty, only contains whitespace, or is
         *             larger than the {@link #MAX_TITLE_LENGTH maximum title size}. The
         *             exception's detail message will be an error message that
        ...
        public EmbedObject getEmbed() {
            ...
            EmbedBuilder builder = new EmbedBuilder().withColor( UserModule.EMBED_COLOR ).withTitle( title )
                    .withDescription( description ).withUrl( url ).withImage( image ).withThumbnail( thumbnail );

            if ( footer != null ) {
                builder.withFooterText( footer ).withFooterIcon( footerIcon );
            }

            if ( author != null ) {
                builder.withAuthorName( author ).withAuthorUrl( authorURL ).withAuthorIcon( authorIcon );
            }
        ...
        /**
         * Checks that the value given for an attribute is within acceptable length. If
         * the value is longer than allowed, throws an exception with the appropriate
         ...
         *            The value received.
         * @param maxLength
         *            The maximum acceptable length for the attribute.
         * @throws IllegalArgumentException
         *             if the value is longer than the maximum acceptable length.
         */
        private static void checkLength( String attribute, String value, int maxLength )
            ...
            if ( value.length() > maxLength ) {
                throw new IllegalArgumentException(
                        String.format( "The %s can only have up to %d " + "characters. Given %s has %d characters.",
                                attribute, maxLength, attribute, value.length() ) );
            }
         ...
         * @throws IllegalArgumentException
         *             if the given description is larger than the
         *             {@link #MAX_DESCRIPTION_LENGTH maximum description size}. The
         *             exception's detail message will be an error message that
         *             indicates what the error was.
         */
         ...
         * 
         * @param url
         *            The URL to be set. If <tt>null</tt>, the current URL is deleted.
         */
         ...
         * 
         * @param thumbnailUrl
         *            The URL of the thumbnail to be set. If <tt>null</tt>, the current
         ...
         *            deleted.
         * @throws IllegalArgumentException
         *             if the given footer is larger than the {@link #MAX_FOOTER_LENGTH
         *             maximum footer size}. The exception's detail message will be an
         *             error message that indicates what the error was.
         */
         ...
         * 
         * @param author
         *            The author to be set. If <tt>null</tt>, the current author is
         *            deleted.
         * @throws IllegalArgumentException
         *             if the given author name is larger than the
         *             {@link #MAX_AUTHOR_LENGTH maximum author size}. The exception's
         *             detail message will be an error message that indicates what the
         *             error was.
         */
         ...
         * @return If <tt>fieldText</tt> is not <tt>null</tt>: <tt>true</tt> if the
         *         field set successfully, <tt>false</tt> if there was no field with the
         *         given name and this card already has the {@link #MAX_FIELDS maximum
         *         amount of fields}.<br>
         *         If <tt>fieldText</tt> is <tt>null</tt>: <tt>true</tt> if the field
         *         with the given name was deleted successfully, <tt>false</tt> if there
         *         was no field with the given name.
         * @throws NullPointerException
         *             if <tt>fieldName</tt> is <tt>null</tt>.
         * @throws IllegalArgumentException
         *             if <tt>fieldName</tt> or <tt>fieldText</tt> is an empty string,
         *             only contains white space, or exceeds the
         *             {@link #MAX_FIELD_NAME_LENGTH maximum name size} or
         *             {@link #MAX_FIELD_TEXT_LENGTH maximum text size}, respectively.
         *             The exception's detail message will be an error message that
         *             indicates what the error was.
         * 
         */
        ...
        private static final Translator<Map<String,
                String>> FIELDS_TRANSLATOR = new MapTranslator<>( new StringTranslator(), new StringTranslator() );
        ...
        /**
         * Loads the card from the given Data. Any current attributes are replaced by
         * the ones specified in the given data.
         * <p>
    ...
    public static class CardEntry implements Storable {
        ...
        /**
         * Matches an entry with the same title. Having a different entry count does not
         * affect the match.
         */
        @Override
    ...
    public static class UserCards implements Storable {
        ...
        private static final Translator<List<CardEntry>> CARDS_TRANSLATOR = new ListTranslator<>(
                new StorableTranslator<>( () -> new CardEntry() ) );

        private int cardAllowance;
        private final List<CardEntry> cards;

        /**
         * Creates an instance with no cards and the {@link #STARTING_CARDS initial
         * allowance} of cards.
         */
        private UserCards() {
        ...
        /**
         * Retrieves the user's card allowance (how many cards the user may have at this
         * time).
         * 
         ...
         * 
         * @return <tt>true</tt> if can still increase. <tt>false</tt> if the user
         *         already has the maximum allowance.
         */
         ...
         * 
         * @return <tt>true</tt> if incremented successfully. <tt>false</tt> if the user
         *         already has the maximum allowance.
         */
        ...
        /**
         * Determines whether this user has the given card (as in a card with the same
         * title).
         * 
         * @param card
         *            The card.
         * @return <tt>true</tt> if the user has the given card. <tt>false</tt>
         *         otherwise.
         */
        ...
        /**
         * Registers a new card for this user, if the user has not yet reached his card
         * allowance.
         * 
         * @param card
         *            The card to add.
         * @return <tt>true</tt> if added successfully. <tt>false</tt> if the user is
         *         already using his full card allowance.
         * @throws IllegalArgumentException
         *             if the user already has a card with the same title. The
         *             exception's detail message will indicate this.
         */
        ...
        /**
         * Deregisters the given card from this user's list (as in a card with the same
         * title).
         * 
         * @param card
         *            The card to remove.
         * @return <tt>true</tt> if removed successfully. <tt>false</tt> if the user
         *         does not have the given card.
         */
        ...
        /**
         * Updates the registration of the given card in this user's list (e.g. updates
         * the field count of the card with the same title).
         * 
         * @param card
         *            The card to update.
         * @return <tt>true</tt> if updated successfully. <tt>false</tt> if the user
         *         does not have the given card.
         */
